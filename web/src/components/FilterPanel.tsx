...
import { HEATMAP_RESOLUTIONS, HeatmapGrid, HeatmapLayer, HeatmapSettings } from "@/lib/heatmap";
import { formatDate } from "@/lib/format";
import { MatchMeta } from "@/lib/types";
import { HeatmapGradientBar, MarkerSwatch } from "./Swatches";
...
interface FilterPanelProps {
  ...
  onHeatmapChange: (next: HeatmapSettings) => void;
...
const HEATMAP_LAYERS: { value: HeatmapLayer; label: string }[] = [
  { value: "off", label: "Off" },
...
const PEAK_UNIT: Record<HeatmapLayer, string> = {
  off: "",
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col gap-5 overflow-y-auto border-l border-zinc-800 bg-zinc-950 p-4 text-sm">
      ...
      <section>
        ...
        <div className="grid grid-cols-4 gap-1 rounded-md bg-zinc-900 p-1">
          {HEATMAP_LAYERS.map((opt) => (
            <button
              key={opt.value}
              onClick={(e) => {
                onHeatmapChange({ ...heatmap, layer: opt.value });
                e.currentTarget.blur();
              }}
              className={`rounded px-1.5 py-1.5 text-xs font-medium ${
                heatmap.layer === opt.value ? "bg-blue-600 text-white" : "text-zinc-300 hover:bg-zinc-800"
              }`}
            >
              {opt.label}
            </button>
          ...
          <div className="mt-3 flex flex-col gap-3">
            <div>
              <div className="mb-1 flex items-center justify-between text-xs text-zinc-400">
                ...
                <span>{heatmap.resolution}×{heatmap.resolution}</span>
              </div>
              <div className="flex gap-1">
                {HEATMAP_RESOLUTIONS.map((r) => (
                  <button
                    key={r}
                    onClick={(e) => {
                      onHeatmapChange({ ...heatmap, resolution: r });
                      e.currentTarget.blur();
                    }}
                    className={`flex-1 rounded px-2 py-1 text-xs ${
                      heatmap.resolution === r ? "bg-blue-600 text-white" : "bg-zinc-800 text-zinc-300 hover:bg-zinc-700"
                    }`}
                  >
                    {r}
                  </button>
            ...
            <div>
              <div className="mb-1 flex items-center justify-between text-xs text-zinc-400">
                <span>Opacity</span>
                <span>{Math.round(heatmap.opacity * 100)}%</span>
              </div>
              <input
                type="range"
                min={0.1}
                ...
                onChange={(e) => onHeatmapChange({ ...heatmap, opacity: Number(e.target.value) })}
                className="w-full"
              />
            </div>

            <div className="rounded-md border border-zinc-800 bg-zinc-900 p-2">
              <div className="rounded bg-zinc-950 p-1">
                <HeatmapGradientBar />
              </div>
